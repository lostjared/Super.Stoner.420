...
static const char *img_str[] = {
    "black.bmp",  "grass.bmp",  "bluebrick.bmp", "bluesky.bmp",
    "brick.bmp",  "eblock.bmp", "red_brick.bmp", "sand1.bmp",
    "sand2.bmp",  "snow.bmp",   "stone.bmp",     "stone2.bmp",
    "stone3.bmp", "stone4.bmp", "black.bmp",     0};
static const char *hstr[] = {"hero1.bmp",      "hero2.bmp",
                             "hero3.bmp",      "hero4.bmp",
                             "hero_jump1.bmp", "hero_shot1.bmp",
                             "hero_shot2.bmp", "hero_shot3.bmp",
                             "hero_shot4.bmp", 0};
...
enum { GAME_WIDTH = 640, GAME_HEIGHT = 480 };
...
#ifdef __EMSCRIPTEN__
  if (custom_level == 0)
    snprintf(sbuf, 255, "/assets/%slevel/level%d.sml", level_str[cur_levels],
             ++cur_level);
#else
  if (custom_level == 0)
    snprintf(sbuf, 255, "%slevel/level%d.sml", level_str[cur_levels],
             ++cur_level);
  ...
  srand((unsigned int)SDL_GetTicks());
  {
    ...
    for (; i < 50; i++) {
      if (level->items[i].type != 0)
        do {
          level->items[i].type = rand() % COLLECT_NUM;
        } while (level->items[i].type == 0);
    }
  }
  offset = 0;
...
static void bind_effect_uniforms(GLuint program, float wallclock_seconds,
                                 float wobble_intensity, float resolution_w,
                                 float resolution_h, int flip_y) {
...
static void render_texture_pass(GLuint program, GLuint source_texture,
                                GLuint target_fbo, int target_viewport_x,
                                int target_viewport_y, int target_viewport_w,
                                int target_viewport_h, float wallclock_seconds,
                                float wobble_intensity, float resolution_w,
                                float resolution_h) {
  int flip_y = (source_texture == gl_tex) ? 0 : 1;

  glBindFramebuffer(GL_FRAMEBUFFER, target_fbo);
  glViewport(target_viewport_x, target_viewport_y, target_viewport_w,
             target_viewport_h);
  ...
  bind_effect_uniforms(program, wallclock_seconds, wobble_intensity,
                       resolution_w, resolution_h, flip_y);
...
static void render_mix_pass(GLuint base_texture, GLuint effect_texture,
                            GLuint target_fbo, int target_viewport_x,
                            int target_viewport_y, int target_viewport_w,
                            int target_viewport_h, float mix_amount) {
...
void activate_collect_shader_effect(int item_type) {
  ...
  if (new_type == COLLECT_SHADER_FX_WOBBLE) {
    effect.wobble_intensity +=
        (float)wobble_count * COLLECT_FX_WOBBLE_INCREMENT;
  ...
  if (collect_fx_count >= COLLECT_FX_MAX_STACK) {
    memmove(collect_fx_stack, collect_fx_stack + 1,
            sizeof(collect_fx_stack[0]) * (COLLECT_FX_MAX_STACK - 1));
...
void reset_collect_shader_effect() { collect_fx_count = 0; }
...
static int init_gl_renderer() {
  ...
  static const char *fs_mix_src =
      ...
      "  color = vec4(mix(base_col, effect_col, clamp(mix_amount, 0.0, 1.0)), "
      "1.0);\n"
      "}\n";
  ...
  static const char *fs_fx_type5_src =
      ...
      "  vec3 lightDir = normalize(vec3(sin(lightAngle), cos(lightAngle), "
      "1.0));\n"
      "  float diff = max(dot(normal, lightDir), 0.0);\n"
      "  float spec = pow(max(dot(reflect(-lightDir, normal), vec3(0.0, 0.0, "
      "1.0)), 0.0), 16.0);\n"
      "  col *= 0.6 + diff * 0.4;\n"
  ...
  static const char *fs_fx_type0_src =
      ...
      "  vec2 p = uv;\n"
      "  for (int i = 0; i < 6; i++) {\n"
      "    p = abs((p - c) * (zoom + 0.15 * sin(t * 0.35 + float(i)))) - 0.5 + "
      "c;\n"
      ...
      "  vec2 ts = 1.0 / res;\n"
      "  vec3 s00 = textureGrad(img, uv + ts * vec2(-1.0,-1.0), dFdx(uv), "
      "dFdy(uv)).rgb;\n"
      "  vec3 s10 = textureGrad(img, uv + ts * vec2( 0.0,-1.0), dFdx(uv), "
      "dFdy(uv)).rgb;\n"
      "  vec3 s20 = textureGrad(img, uv + ts * vec2( 1.0,-1.0), dFdx(uv), "
      "dFdy(uv)).rgb;\n"
      "  vec3 s01 = textureGrad(img, uv + ts * vec2(-1.0, 0.0), dFdx(uv), "
      "dFdy(uv)).rgb;\n"
      "  vec3 s11 = textureGrad(img, uv,                        dFdx(uv), "
      "dFdy(uv)).rgb;\n"
      "  vec3 s21 = textureGrad(img, uv + ts * vec2( 1.0, 0.0), dFdx(uv), "
      "dFdy(uv)).rgb;\n"
      "  vec3 s02 = textureGrad(img, uv + ts * vec2(-1.0, 1.0), dFdx(uv), "
      "dFdy(uv)).rgb;\n"
      "  vec3 s12 = textureGrad(img, uv + ts * vec2( 0.0, 1.0), dFdx(uv), "
      "dFdy(uv)).rgb;\n"
      "  vec3 s22 = textureGrad(img, uv + ts * vec2( 1.0, 1.0), dFdx(uv), "
      "dFdy(uv)).rgb;\n"
      "  return (s00 + 2.0*s10 + s20 + 2.0*s01 + 4.0*s11 + 2.0*s21 + s02 + "
      "2.0*s12 + s22) / 16.0;\n"
      "}\n"
      ...
      "  vec2 q = abs(pv);\n"
      "  if (q.y > q.x) q = q.yx;\n"
      "  float base = 1.82 + 0.18 * pingPong(sin(time_f * 0.2) * (PI * "
      "time_f), 5.0);\n"
      "  float period = log(base) * pingPong(time_f * PI, 5.0);\n"
      "  float tz = time_f * 0.65;\n"
      "  float rD = diamondRadius(pv) + 1e-6;\n"
      "  float ang = atan(q.y, q.x) + tz * 0.35 + 0.35 * sin(rD * 18.0 + "
      "time_f * 0.6);\n"
      ...
      "  vec2 off = dir * (0.0015 + 0.001 * sin(time_f * 1.3)) * vec2(1.0, 1.0 "
      "/ aspect);\n"
      ...
      "  float ring = smoothstep(0.0, 0.7, sin(log(rD + 1e-3) * 9.5 + time_f * "
      "1.2));\n"
      ...
      "  vec3 bloom = outCol * outCol * 0.18 + pow(max(outCol - 0.6, "
      "vec3(0.0)), vec3(2.0)) * 0.12;\n"
      "  outCol += bloom;\n"
      "  outCol = mix(outCol, baseCol, pingPong(pulse * PI, 5.0) * 0.18);\n"
      "  outCol = clamp(outCol, vec3(0.05), vec3(0.97));\n"
      "  vec3 finalRGB = mix(baseTex.rgb, outCol, pingPong(glow * PI, 5.0) * "
      "0.8);\n"
  ...
  static const char *fs_fx_item5_src =
      ...
      "  float ripple = sin(v_uv.x * rippleWavelength + time_f * rippleSpeed) "
      "* rippleAmplitude;\n"
      "  ripple += sin(v_uv.y * rippleWavelength + time_f * rippleSpeed) * "
      "rippleAmplitude;\n"
      ...
      "  vec2 twistedUV = (rotationMatrix * (v_uv - vec2(0.5, 0.5))) + "
      "vec2(0.5, 0.5);\n"
      "  vec4 originalColor = texture(samp, v_uv);\n"
      "  vec4 twistedRippleColor = texture(samp, mix(rippleUV, twistedUV, "
      "0.5));\n"
      "  color = mix(originalColor, twistedRippleColor, 0.5);\n"
      "}\n";

  static const float quad[] = {-1.0f, 1.0f,  0.0f, 0.0f, -1.0f, -1.0f,
  ...
  if (!gl_program || !gl_fx_mix_program || !gl_fx_program ||
      !gl_fx_type5_program || !gl_fx_type0_program || !gl_fx_item5_program)
    return -1;
...

  {
    ...
    for (; ping_index < 2; ping_index++) {
      ...
      glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
                             GL_TEXTURE_2D, gl_pingpong_tex[ping_index], 0);
  ...
  gl_fx_type5_resolution_loc =
      glGetUniformLocation(gl_fx_type5_program, "iResolution");
  ...
  gl_fx_type0_resolution_loc =
      glGetUniformLocation(gl_fx_type0_program, "iResolution");
...
static void present_frame() {
  ...
  glActiveTexture(GL_TEXTURE0);
  glBindTexture(GL_TEXTURE_2D, gl_tex);
  glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, front->w, front->h, GL_RGBA,
  ...
  if (cur_scr == ID_CREDITS) {
    ...
    render_texture_pass(gl_fx_program, source_texture, 0, viewport_x,
                        viewport_y, viewport_w, viewport_h, wallclock_seconds,
                        wobble_to_upload, (float)GAME_WIDTH,
                        (float)GAME_HEIGHT);
  } else {
    int effect_index = 0;
    int ping_index = 0;
    ...
    if (collect_fx_count == 0) {
      render_texture_pass(gl_program, source_texture, 0, viewport_x, viewport_y,
                          viewport_w, viewport_h, wallclock_seconds, 0.0f,
      ...
      for (; effect_index < collect_fx_count; effect_index++) {
        ...
        render_texture_pass(
            collect_effect_program(effect->type), source_texture, target_fbo, 0,
            0, GAME_WIDTH, GAME_HEIGHT, wallclock_seconds,
            effect->wobble_intensity, (float)GAME_WIDTH, (float)GAME_HEIGHT);
      ...
      if (final_mix_amount < 0.999f) {
        ...
        render_texture_pass(gl_program, gl_tex, gl_pingpong_fbo[ping_index], 0,
                            0, GAME_WIDTH, GAME_HEIGHT, wallclock_seconds, 0.0f,
                            (float)GAME_WIDTH, (float)GAME_HEIGHT);
        render_mix_pass(base_texture, source_texture, 0, viewport_x, viewport_y,
                        viewport_w, viewport_h, final_mix_amount);
      } else {
        render_texture_pass(gl_program, source_texture, 0, viewport_x,
                            viewport_y, viewport_w, viewport_h,
                            wallclock_seconds, 0.0f, (float)GAME_WIDTH,
                            (float)GAME_HEIGHT);
      }
    }
  }
...
static void rls() {
  ...
  if (gl_tex || gl_pingpong_tex[0] || gl_pingpong_tex[1]) {
    GLuint textures_to_delete[3] = {gl_tex, gl_pingpong_tex[0],
                                    gl_pingpong_tex[1]};
...
int main(int argc, char **argv) {
  ...
  if (ico == NULL) {
    fprintf(stderr, "Error loading icon, wrong path place this program in the "
...
#define GL_FRAMEBUFFER_SRGB_EXT 0x8DB9
...

  {
    ...
    for (controller_index = 0; controller_index < SDL_NumJoysticks();
         controller_index++) {
  ...
  front = SDL_CreateRGBSurfaceWithFormat(0, GAME_WIDTH, GAME_HEIGHT, 32,
...
void SDL_ReverseBlt(SDL_Surface *surf, SDL_Rect *rc, SDL_Surface *front_surf,
                    SDL_Rect *rc2, Uint32 transparent) {
  void *buf, *buf2;
  int i, z, i2, z2;
  buf = lock(surf);
  ...
  for (i = rc->w - 1; i > 0; i--) {
    for (z = 0; z < rc->h; z++) {
      ...
      if (color != transparent)
        setpixel(buf2, i2, z2,
                 SDL_MapRGB(front_surf->format, col.r, col.g, col.b),
                 front_surf->format->BitsPerPixel, front_surf->pitch);
      z2++;
    }
    z2 = rc2->y;
    i2++;
...
int SDL_Colide(SDL_Rect *rc, SDL_Rect *rc2) {
  ...
  for (i = rc->x; i < rc->x + rc->w; i++) {
    for (z = rc->y; z < rc->y + rc->h; z++) {
      if (i >= rc2->x && i <= rc2->x + rc2->w && z >= rc2->y &&
          z <= rc2->y + rc2->h)
        return 1;
    }
  }
