...
Uint32 check_start_in(Uint32 i, void *v) {
  ...
  if (controller_button(SDL_CONTROLLER_BUTTON_DPAD_UP) || axis2 < -8000) {
    ...
    if (menu_level == 1 && cl2_pos > 0)
      cl2_pos--;
  } else if (controller_button(SDL_CONTROLLER_BUTTON_DPAD_DOWN) ||
             axis2 > 8000) {
...
void render_start() {
  ...
  if (shown_logo == 1) {
    ...
    if (menu_level == 0) {
      SDL_PrintText(
          front, font, 75, 75,
          SDL_MapRGB(front->format, rand() % 255, rand() % 255, rand() % 255),
          "Super Stoner 420-LostSideDead");
      SDL_PrintText(front, font, 125, 175,
                    SDL_MapRGB(front->format, 255, 255, 255), "Single Player ");
      SDL_PrintText(front, font, 125, 175 + 50,
                    SDL_MapRGB(front->format, 255, 0, 0), "Credits");
      SDL_PrintText(front, font, 125, 175 + 100,
                    SDL_MapRGB(front->format, 255, 255, 0), "Exit");
      {
        SDL_Rect rc = {125, front->h - 100 - hgfx[0]->h, hgfx[0]->w,
                       hgfx[0]->h};
        SDL_SetColorKey(hgfx[0], SDL_TRUE,
                        SDL_MapRGB(hgfx[0]->format, 255, 255, 255));
        SDL_BlitSurface(hgfx[0], 0, front, &rc);
        {
          int cx = 100, cy = 175;
          cy = cy + (50 * cl_pos);
          SDL_PrintText(front, font, cx, cy,
                        SDL_MapRGB(front->format, rand() % 255, rand() % 255,
                                   rand() % 255),
                        "=)>");
        }
      }
      ...
      SDL_PrintText(
          front, font, 75, 75,
          SDL_MapRGB(front->format, rand() % 255, rand() % 255, rand() % 255),
          "Super Stoner 420 Play New Game");
      SDL_PrintText(front, cfont, 125, 175,
                    SDL_MapRGB(front->format, 255, 255, 255),
                    "New Super Stoner 420 Levels");
      SDL_PrintText(front, cfont, 125, 175 + 50,
                    SDL_MapRGB(front->format, 255, 0, 0),
                    " Old SuperMaster2 Levels");
      {
        SDL_Rect rc = {125, front->h - 100 - hgfx[0]->h, hgfx[0]->w,
                       hgfx[0]->h};
        SDL_SetColorKey(hgfx[0], SDL_TRUE,
...
void render_enter_level() {
  SDL_BlitSurface(bg, 0, front, 0);
  {
    ...
    SDL_BlitSurface(hgfx[0], 0, front, &rc2);
    {
      ...
      SDL_PrintText(front, font, 150, 100,
                    SDL_MapRGB(front->format, 255, 255, rand() % 255), sbuf);
      SDL_PrintText(front, cfont, 150, 175,
                    SDL_MapRGB(front->format, 255, 255, 255), lifebuf);
      SDL_PrintText(
          front, font, 150, 200,
          SDL_MapRGB(front->format, rand() % 255, rand() % 255, rand() % 255),
          "Press Start to Play Level");
      SDL_PrintText(
          front, font, 150, 200,
          SDL_MapRGB(front->format, rand() % 255, rand() % 255, rand() % 255),
          "Press Start to Play Level");
...
static void credits_in() {
  const Uint8 *keys = SDL_GetKeyboardState(0);
  if (keys[SDL_SCANCODE_RETURN] || controller_button(SDL_CONTROLLER_BUTTON_B) ||
...
void render_credits() {
  ...
  for (i = 0; i < front->w; i++) {
    for (z = 0; z < front->h; z++) {
      ...
      color = SDL_MapRGB(front->format,
                         ((Uint8)(alpha * col1.r) + (1 - alpha) * col2.r),
                         (Uint8)(alpha * col1.g) + (1 - alpha) * col2.g,
                         (Uint8)(alpha * col1.b) + (1 - alpha) * col2.b);
      if ((rand() % 2) == 0)
        color += (int)sinf(alpha) * col1.r / (1 + col2.g);
      else
        color -= (int)cosf(alpha) * col1.b / (1 + col1.r);
    ...
    alpha = 0.0f;
  {
    ...
    SDL_FillRect(front, &rc, 0);
    SDL_SetColorKey(hgfx[4], SDL_TRUE,
                    SDL_MapRGB(front->format, 255, 255, 255));
    SDL_BlitSurface(hgfx[4], 0, front, &rcX);
    {
      SDL_Rect rcY = {0, 0, evil_gfx[0].gfx[0]->w, evil_gfx[0].gfx[0]->h};
      SDL_Rect rcZ = {640 - 120 - evil_gfx[0].gfx[0]->w, 315,
                      evil_gfx[0].gfx[0]->w, evil_gfx[0].gfx[0]->h};
    ...
    SDL_PrintText(front, font, 170, 310,
                  SDL_MapRGB(front->format, 255, 255, 255),
                  "Super Stoner 420 - LostSideDead");
    SDL_PrintText(
        front, cfont, 170, 340,
