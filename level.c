...
static int gfx_index_valid(int idx) {
  return (idx >= 0 && idx < (int)(sizeof(gfx) / sizeof(gfx[0])) &&
          gfx[idx] != NULL);
}

static int collect_index_valid(int idx) {
  return (idx >= 0 && idx < (int)(sizeof(collect) / sizeof(collect[0])) &&
          collect[idx] != NULL);
}
...
void release_level(Level *lvl) { free(lvl); }

void render_pause() {
  SDL_Rect rc = {50, 50, 640 - 100, 480 - 100};

  if (controller_button(SDL_CONTROLLER_BUTTON_A) ||
      controller_button(SDL_CONTROLLER_BUTTON_START)) {
    cur_scr = ID_GAME;
  }
  if (controller_button(SDL_CONTROLLER_BUTTON_Y) ||
  ...
  SDL_PrintText(front, font, 75, 75, SDL_MapRGB(front->format, 255, 255, 255),
                "Paused - Press A or Start to continue ");
  SDL_PrintText(front, font, 75, 110, SDL_MapRGB(front->format, 255, 0, 0),
                "Press Y or Back to Return to Menu ");
}
...
void render_map(SDL_Surface *surf, Level *lvl) {
  ...
  for (i = 0; i < 700 - 4 + 24; i++) {
    ...
    if (idx < 0 || idx >= MAX_TILE) {
      bx += 16;
      by = (((++gcount) > 23) ? (startby + ((gcount = 0), (bx += 16, 0))) : by);
    } else {
  ...
  SDL_PrintText(surf, font, 25, 25, SDL_MapRGB(surf->format, 255, 255, 255),
                lvl->level_name);
  {
    static char buf[256];
    memset(buf, 0, sizeof(buf));
    snprintf(buf, 255, "Score: %d Lives: %d", score, lives);
    SDL_PrintText(surf, cfont, 350, 25,
                  SDL_MapRGB(surf->format, rand() % 255, rand() % 255, 255),
                  buf);
  }
  startby = 75;
  bx = 75;
  ...
  if (hero.cur_ani < 0 ||
      hero.cur_ani >= (int)(sizeof(hgfx) / sizeof(hgfx[0]))) {
  ...
  for (i = 0; i < 700 - 4 + 24; i++) {
    if (!hgfx[hero.cur_ani]) {
      ...
      if ((int)i == hero.hpos) {
        hero.x = bx;
        hero.y = by;
        SDL_SetColorKey(hgfx[hero.cur_ani], SDL_TRUE,
                        SDL_MapRGB(hgfx[hero.cur_ani]->format, 255, 255, 255));
        if (hero.dir == 1)
          SDL_BlitSurface(hgfx[hero.cur_ani], 0, surf, &rc);
        else {
          SDL_Rect rc2 = {1, 1, hgfx[hero.cur_ani]->w - 1,
                          hgfx[hero.cur_ani]->h - 1};
          SDL_ReverseBlt(hgfx[hero.cur_ani], &rc2, surf, &rc,
                         SDL_MapRGB(hgfx[hero.cur_ani]->format, 255, 255, 255));
        }
      }
    }

    {
      ...
      for (pos = 0; pos < MAX_PARTICLE; pos++) {
        if (pos < MAX_PARTICLE && emiter.p[pos].type != 0 &&
            emiter.p[pos].vpos == (int)i + offset) {
          if (particles[0]) {
            SDL_Rect rcX = {bx, by, particles[0]->w, particles[0]->h};
            SDL_SetColorKey(particles[0], SDL_TRUE,
                            SDL_MapRGB(particles[0]->format, 255, 255, 255));
      ...
      for (pos = 0; pos < 50; pos++) {
        if (pos < 50 && evil[pos].type != -1 &&
            evil[pos].vpos == (int)i + offset) {
          if (evil[pos].egfx && evil[pos].cur_ani >= 0 &&
              evil[pos].cur_ani < 8) {
            int ani = evil[pos].cur_ani;
            if (evil[pos].egfx->gfx[ani]) {
              SDL_Rect erc = {0, 0, evil[pos].egfx->gfx[ani]->w - 1,
                              evil[pos].egfx->gfx[ani]->h};
              SDL_Rect prc = {bx, by, evil[pos].egfx->gfx[ani]->w,
                              evil[pos].egfx->gfx[ani]->h};
              if (evil[pos].dir == 1) {
                SDL_SetColorKey(evil[pos].egfx->gfx[ani], SDL_TRUE,
                                SDL_MapRGB(evil[pos].egfx->gfx[ani]->format,
                                           255, 255, 255));
                SDL_BlitSurface(evil[pos].egfx->gfx[ani], 0, surf, &prc);
              } else {
                SDL_ReverseBlt(evil[pos].egfx->gfx[ani], &erc, surf, &prc,
                               SDL_MapRGB(evil[pos].egfx->gfx[ani]->format, 255,
                                          255, 255));
              }
              evil[pos].x = bx;
              evil[pos].y = by;
        ...
        if (pos < 50 && level->items[pos].type != 0 &&
            level->items[pos].vpos == (int)i + offset) {
          int t = level->items[pos].type;
          if (collect_index_valid(t)) {
            SDL_Rect rc = {bx, by, collect[t]->w, collect[t]->h};
            SDL_SetColorKey(collect[t], SDL_TRUE,
                            SDL_MapRGB(collect[t]->format, 255, 255, 255));
...
static void move_left() {
  ...
  if (hero.hpos > 0 && offset == 0) {
    if (hero.hpos >= 24) {
      Uint8 check[4];
      int a0 = hero.hpos - 24, a1 = hero.hpos + 1 - 24, a2 = hero.hpos + 2 - 24,
          a3 = hero.hpos + 3 - 24;
...
static int check_input() {
  ...
  if ((keys[SDL_SCANCODE_A] || controller_button(SDL_CONTROLLER_BUTTON_A)) &&
      jump_ok == 1 && jump == 0)
    jump = 1, shoot_ani = 0;

  if ((keys[SDL_SCANCODE_S] || controller_button(SDL_CONTROLLER_BUTTON_X)) &&
      jump_ok == 1 && jump == 0) {
    if (shoot_ani == 0) {
  ...
  if ((keys[SDL_SCANCODE_LEFT] ||
       controller_button(SDL_CONTROLLER_BUTTON_DPAD_LEFT) || axis_x < -8000)) {
#endif
    move_left();
    return 0;
  }
  ...
  if ((keys[SDL_SCANCODE_RIGHT] ||
       controller_button(SDL_CONTROLLER_BUTTON_DPAD_RIGHT) || axis_x > 8000)) {
#endif
    move_right();
    return 0;
  }
...
static void proc_collect() {
  ...
  for (i = 0; i < 50; i++) {
    if (level->items[i].type != 0) {
      unsigned int z = 0;
      for (z = 0; z < 4; z++) {
        if ((offset + hero.hpos + 24 + z == level->items[i].vpos) ||
            (offset + hero.hpos - 24 - z == level->items[i].vpos) ||
            (offset + hero.hpos + z == level->items[i].vpos) ||
            offset + hero.hpos - z == level->items[i].vpos) {
...
void init_particles(Emiter *e) { memset(e, 0, sizeof(Emiter)); }
  ...
  for (i = 0; i < 50; i++) {
    ...
    if (evil[i].type != -1) {
      ...
      int horizontal_distance =
          abs((int)evil[i].vpos - (int)(hero.hpos + offset));

      int level_width = 24;

      int enemy_tile_x = (evil[i].vpos >= 0) ? (evil[i].vpos % level_width) : 0;
      int enemy_tile_y = (evil[i].vpos >= 0) ? (evil[i].vpos / level_width) : 0;
      int hero_tile_x = ((hero.hpos + offset) >= 0)
                            ? ((hero.hpos + offset) % level_width)
                            : 0;
      int hero_tile_y = ((hero.hpos + offset) >= 0)
                            ? ((hero.hpos + offset) / level_width)
                            : 0;
      ...
      int is_aggressive = (horizontal_distance < 96) &&
                          (vertical_distance < 3) && !lost_focus[i];
      ...
      if (evil[i].dir == 0) {
        ...
        if (!left_check[0] && !left_check[1] && !left_check[2] &&
            !left_check[3])
      ...
      if (evil[i].type != -1 && hero.x > 0 && hero.y > 0 && evil[i].x > 0 &&
          evil[i].y > 0) {
        int vertical_pixel_distance = abs(hero.y - evil[i].y);
        if (vertical_pixel_distance < 32) {
          if (evil[i].egfx && evil[i].cur_ani >= 0 && evil[i].cur_ani < 8 &&
              evil[i].egfx->gfx[evil[i].cur_ani] && hero.cur_ani >= 0 &&
              hero.cur_ani < (int)(sizeof(hgfx) / sizeof(hgfx[0])) &&
              hgfx[hero.cur_ani]) {

            SDL_Rect rcY = {evil[i].x + 2, evil[i].y + 2,
                            evil[i].egfx->gfx[evil[i].cur_ani]->w - 4,
                            evil[i].egfx->gfx[evil[i].cur_ani]->h - 4};
            SDL_Rect rcX = {hero.x + 2, hero.y + 2, hgfx[hero.cur_ani]->w - 4,
                            hgfx[hero.cur_ani]->h - 4};
...

      {
        ...
        for (; p < MAX_PARTICLE; p++) {
          if (e->p[p].type != 0) {
            if (e->p[p].x > 0 && e->p[p].y > 0 && e->p[p].x < 640 &&
                e->p[p].y < 480 && evil[i].x > 0 && evil[i].y > 0 &&
                evil[i].x < 640 && evil[i].y < 480 && particles[0] != NULL) {

              if (evil[i].egfx && evil[i].cur_ani >= 0 && evil[i].cur_ani < 8 &&
                  evil[i].egfx->gfx[evil[i].cur_ani]) {

                SDL_Rect rcX = {e->p[p].x, e->p[p].y, particles[0]->w,
                                particles[0]->h};
                SDL_Rect rcY = {evil[i].x, evil[i].y,
                                evil[i].egfx->gfx[evil[i].cur_ani]->w,
                                evil[i].egfx->gfx[evil[i].cur_ani]->h};
                if (SDL_Colide(&rcX, &rcY)) {
                  e->p[p].type = 0;
                  evil[i].die = 1;
                  evil[i].cur_ani = 5;
